    ...
    <div className="h-full bg-slate-900 border border-slate-800 rounded-xl flex flex-col overflow-hidden">
      <div className="p-3 border-b border-slate-800 bg-slate-950/50 flex justify-between items-center">
        <h3 className="text-xs font-bold text-slate-300 uppercase tracking-widest flex items-center gap-2">
          <Database className="h-4 w-4 text-blue-500" /> PostgreSQL Node
        </h3>
      ...
      <div className="flex-1 p-3 grid grid-cols-2 gap-3">
        ...
        <div className="bg-slate-950 rounded p-2 border border-slate-800">
          <span className="block text-[10px] text-slate-500 uppercase tracking-widest mb-1">Database</span>
          <span className="text-sm font-mono text-slate-300">{data.database}</span>
        </div>
        
        <div className="col-span-2 mt-2 space-y-1">
          <div className="flex justify-between text-xs border-b border-slate-800 pb-1">
            <span className="text-slate-400">Crimes</span>
            <span className="font-mono text-slate-200">{data.crime_records}</span>
          </div>
          <div className="flex justify-between text-xs border-b border-slate-800 pb-1">
            <span className="text-slate-400">Suspects</span>
            <span className="font-mono text-slate-200">{data.suspects}</span>
          </div>
          <div className="flex justify-between text-xs border-b border-slate-800 pb-1">
            <span className="text-slate-400">Vehicles</span>
            <span className="font-mono text-slate-200">{data.vehicles}</span>
          </div>
          <div className="flex justify-between text-xs">
            <span className="text-slate-400">Alerts</span>
            <span className="font-mono text-slate-200">{data.alerts}</span>
          </div>
        </div>
      </div>
    </div>
